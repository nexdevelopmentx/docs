...
Entrances, exits, room prices, door props, blips, peds, elevator stops — all of it is set in the database via `/hoteladmin` (or by running a [bundled import](/nexdev_hotels/installation#pre-configured-hotels)). There is no hotel list in `config.lua`.

See [Admin panel](/nexdev_hotels/admin-panel) for the in-game workflow.
