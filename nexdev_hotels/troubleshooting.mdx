...
description: "If something's not working, check the F8 console (client) and your server console, filtered for [nexdev_hotels]. Most issues print a clear error there."
---

If something's not working, check the **F8** console (client) and your **server console**, filtered for `[nexdev_hotels]`. Most issues print a clear error there.
...
| Players fall through the map when entering a room | The MLO or IPL for that hotel isn't streamed on your server. You need to own and load the matching interior — see [pre-configured hotels](/nexdev_hotels/installation#pre-configured-hotels) for links. |
...
| Money is charged but the rental doesn't activate | Wrong `Config.Framework`, or `nexdev_hotels` is starting before your core resource. Check `server.cfg` order. |
...
4. Server console output filtered for `[nexdev_hotels]` from when the issue happens.
5. F8 console output filtered for `[nexdev_hotels]`.
