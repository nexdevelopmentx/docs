...
| Problem | Fix |
| --- | --- |
| **Using the crutch item does nothing** | The inventory `useCrutch` hook isn't wired up. For `ox_inventory`, check the item entry has `server.export = 'nex_crutchsystem.useCrutch'`. For QBCore / Qbox, make sure you've registered the `CreateUseableItem` callback. See [Installation → Inventory item](/nex-crutchsystem/installation#inventory-item). |
...
| **Legacy crutch script's crutch isn't removed at the NPC** | The entry in `Config.CrutchResources` is wrong, or the legacy script isn't running. Each entry needs a `resource` name and **either** an `event` or `export`. See [Configuration → Legacy crutch resources](/nex-crutchsystem/configuration#legacy-crutch-resources). |
