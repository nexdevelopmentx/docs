...
Both also work from the server console / chat as the `/givecoins` and `/setcoins` commands — see [Commands](/nexdev_donatorsystem/commands).
...
Every action you take in the Admin panel — create, edit, delete, give, set, theme change — is posted to the `admin` Discord webhook if it's configured in `configs/sv_config.lua`. See [Configuration → Discord webhooks](/nexdev_donatorsystem/configuration#discord-webhooks).
