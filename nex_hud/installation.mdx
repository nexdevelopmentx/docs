...
Release builds ship with the NUI already compiled in `dist/`. If you cloned the repository instead and see this in your server console:

```
UI not compiled, either compile the UI or download a compiled version
```

build the interface with [Bun](https://bun.sh/) (or Node.js):
