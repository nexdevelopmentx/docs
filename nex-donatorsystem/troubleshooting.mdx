...
| `Tebex secret not configured (sv_tebexSecret convar)` | You haven't set the secret. Add `sv_tebexSecret "..."` to your `server.cfg` above `ensure nex_donatorsystem` and restart. See [Tebex setup](/nex-donatorsystem/tebex-setup#step-1--set-your-tebex-secret). |
...
| Order redeems for 0 currency | The package ID isn't in `ServerConfig.CoinPackages` and `ServerConfig.CoinPerDollar` is `0`. Either add the package to the map, or set a fallback rate. See [Tebex setup → Step 3](/nex-donatorsystem/tebex-setup#step-3--map-packages-to-currency). |
...
| Vehicle spawns but I have no keys | Either no keys script is detected, or you've set `Config.GiveVehicleKeys = false`. List of supported keys scripts is in [Configuration → Vehicle keys](/nex-donatorsystem/configuration#vehicle-keys). |
