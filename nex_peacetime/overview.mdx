...
Built on `ox_lib` and OneSync GlobalState — the server is the single source of truth, so restriction changes replicate to every player instantly without a resource restart.
