...
CinematicGarage = {
  ...
  transitionMs = 650,
  switchMs = 350,
  screenEffects = true, -- vignette + bloom while browsing, bright pulse on exit
...
camera. **← / →** browse, **E / ENTER** drives, **BACKSPACE / ESC** exits. A
slim HUD shows the vehicle's name, plate, condition, mileage and class.
...
panel still opens when nothing is parked at the garage — so vehicles stored
elsewhere stay reachable for transfers.
