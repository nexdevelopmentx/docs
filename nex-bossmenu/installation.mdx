...
4. Edit `config.lua` as needed — see [Configuration](/nex-bossmenu/configuration).
...
The framework line is the most useful one — if it doesn't match the framework you actually run, set `Config.Framework` explicitly. See [Configuration → Framework](/nex-bossmenu/configuration#framework).
