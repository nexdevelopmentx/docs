...
description: "Filter your server console for [nex_queue] — most issues print a clear error there. If you're not sure, flip Config.Server.DebugMode = true temporarily, restart, and try again."
---
...
| Everyone is queued as `Standard` with priority 0 | **Server Members Intent** is OFF. Turn it on in the bot's settings (see [Discord setup, step 2](/nex-queue/discord-setup#2-enable-the-right-intents)) and restart the resource. |
...
5. **Did you restart the resource after editing the config?** Most settings need a restart. (Themes don't — see [Themes](/nex-queue/themes#hot-swap-at-runtime).)
