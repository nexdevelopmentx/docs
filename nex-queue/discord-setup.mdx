...
Head back to [Installation — Step 4](/nex-queue/installation#step-4-fill-in-config-lua) to paste these into `config.lua`. The full mapping of every config key lives in [Configuration](/nex-queue/configuration).
