...
That's the whole list — no framework, no SQL, no other resources. The web UI ships pre-built in `web/dist`, so there's nothing to compile.
...
<Warning>
If you customised the web UI, the source lives in `web/`. Run `npm install` then `npm run build` inside `web/` to regenerate `web/dist` — the game only loads the built files, not the source.
</Warning>
