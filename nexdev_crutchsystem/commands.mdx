...
| `/crutchadmin` | Opens the admin panel. The server-side `nexdev_crutchsystem:admin:list` callback rejects non-admins. |
...
The admin panel shows every currently-forced player with the source label (e.g. `EMS`, or whatever you pass to `ForceCrutch`), seconds remaining, and a one-click clear button. The same data is available via the [`ListForced()`](/nexdev_crutchsystem/exports) export if you want to surface it in your own UI.
