...
description: "Filter the server console and the F8 client console for nexdev_crutchsystem first."
---
...
| Problem | Fix |
| --- | --- |
| **Using the crutch item does nothing** | The inventory `useCrutch` hook isn't wired up. For `ox_inventory`, check the item entry has `server.export = 'nexdev_crutchsystem.useCrutch'`. For QBCore / Qbox, make sure you've registered the `CreateUseableItem` callback. See [Installation → Inventory item](/nexdev_crutchsystem/installation#inventory-item). |
...
| **Money is charged but the crutch stays** | The server's `selfRemoveInternal` step failed after `chargeMoney` succeeded. Filter the server console for `nexdev_crutchsystem` — you should see the error. Refund and report. |
...
| **Legacy crutch script's crutch isn't removed at the NPC** | The entry in `Config.CrutchResources` is wrong, or the legacy script isn't running. Each entry needs a `resource` name and **either** an `event` or `export`. See [Configuration → Legacy crutch resources](/nexdev_crutchsystem/configuration#legacy-crutch-resources). |
