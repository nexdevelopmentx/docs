...
You can also pre-seed display cars in `shared/locations.lua` via the `DisplayVehicles = { ... }` field — see [Dealership lots](/nex-dealerships/dealerships#display-vehicles).
...
Per-lot toggles persisted in `nex_dealership_settings.settings` (JSON blob). These override the global defaults for this lot — useful for, say, disabling buybacks at a luxury showroom while keeping them on at the truck lot.
