---
title: "Configuration"
description: "Everything lives in config.lua. Edit, then restart nexdev_bossmenu."
---
...
Admin status is granted if **any** of these match. `group.admin` / `group.superadmin` are the built-in txAdmin / Cfx ACE groups, so most servers get admin access for free without touching `permissions.cfg`. See [Commands → Permissions](/nexdev_bossmenu/commands#permissions) for the full breakdown.
